...
class WebController extends Controller
{
    ...
    public function careers()
    {
        $data['title']='Careers';
        $data['value'] = About::first();
        return view('admin.web-control.careers', $data);
    }
    
    public function kyc()
    {
        $data['title']='Know Your Customers';
        $data['value'] = About::first();
        return view('admin.web-control.kyc', $data);
    }
    ...
    public function Updatehomepage(Request $request)
    {
        ...
        $data->s9_title=$request->s9_title;
        $data->s9_body=$request->s9_body;
        $res=$data->save();
        if ($res) {
            return back()->with('success', 'Update was Successful!');
        } else {
    ...
    public function section8(Request $request)
    {
        ...
        $data = Design::find(1);
        if($request->hasFile('section8')){
            $image = $request->file('section8');
            $filename = 'section8_'.time().'.png';
            $location = 'asset/images/' . $filename;
            Image::make($image)->save($location);
            $path = './asset/images/';
            File::delete($path.$data->s8_image);
            $data['s8_image'] = $filename;
        }
        $res = $data->save();
        if ($res) {
            return back()->with('success', 'Updated Successfully!');
    ...
    public function section9(Request $request)
    {
        ...
        $data = Design::find(1);
        if($request->hasFile('section9')){
            $image = $request->file('section9');
            $filename = 'section9_'.time().'.png';
            $location = 'asset/images/' . $filename;
            Image::make($image)->save($location);
            $path = './asset/images/';
            File::delete($path.$data->s9_image);
            $data['s9_image'] = $filename;
        }
        $res = $data->save();
        if ($res) {
            return back()->with('success', 'Updated Successfully!');
