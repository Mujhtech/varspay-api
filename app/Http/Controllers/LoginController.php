...
class LoginController extends Controller
{
        ...
        public function submitlogin(Request $request)
    {
    	...
    	if(Auth::attempt(['email' => $request->email,'password' => $request->password,])){
            ...
            if($user->session_flag != '') {
                $last_session = \Session::getHandler()->read($user->session_flag);
                if ($last_session) {
                    if (\Session::getHandler()->destroy($user->session_flag)) {
            ...
            event(new SystemLogEvent("You logged in", $user->id));
